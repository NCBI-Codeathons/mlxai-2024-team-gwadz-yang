...
output_file = os.path.join(DATA_DIR, 'Matrix_CurName_simplified_SpecifiedArchs_SuperFams_one_hot_encoding.tsv')
...
with open(output_file, 'w') as f:
    ...
    for index, row in df.iterrows():
        ...
        cur_name = row['CurName_simplified']
        f.write(cur_name)
        f.write('\t')
        ...
        if not pd.isna(super_fams):
            for sf in super_fams.split(' '):
                arr[all_superfamilies.index(sf.lower())] = 1
